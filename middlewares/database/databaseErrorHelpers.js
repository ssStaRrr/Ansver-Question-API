...
const checkQuestionExist = asyncErrorWrapper(async(req,res,next) => {
    ...
    if(!question) return next(new CustomError("There is not such with that question",400));
    next();
});
...
const checkQuestionAndAnswerExist = asyncErrorWrapper(async(req,res,next) => {
    ...
    if(!answer) return next(new CustomError("There is not such with that id associated with question id",400));
    next();
});
